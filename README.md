...
- 모바일 우선 반응형 + 하단 고정 [신청하기] 버튼 (신청 폼이 보이면 자동으로 내려감)
- 320~430px 전 구간 가로 스크롤 0, 탭 영역 44px 이상 확보
- 카카오톡 친구 추가 — ID 복사 후 앱 열기 + 3단계 안내
- 라이트/다크 모드 자동 대응, 본문 18px·버튼 높이 56px 이상 (40~60대 가독성)
- 폰 화면에서 섹션이 구분되도록 바탕색 조정 (흰 패널 대비 1.08 → 1.28)
